...
export default function AdminRoomsTable({ rooms }: { rooms: RoomWithOwner[] }) {
  ...
  async function handleLockTopic(topicId: string, isLocked: boolean, roomId: string, slug: string) {
    ...
    if (result?.error) setError(result.error)
      ...
      setRoomDetails(prev => ({
        ...prev,
        [roomId]: { ...prev[roomId], topics: prev[roomId].topics.map(t => t.id === topicId ? { ...t, is_locked: !isLocked } : t) }
      }))
    }
    setLoading(null)
  }
  ...
  async function handlePinTopic(topicId: string, isPinned: boolean, roomId: string, slug: string) {
    ...
    if (result?.error) setError(result.error)
      ...
      setRoomDetails(prev => ({
        ...prev,
        [roomId]: { ...prev[roomId], topics: prev[roomId].topics.map(t => t.id === topicId ? { ...t, is_pinned: !isPinned } : t) }
      }))
    }
    setLoading(null)
  }
              ...
              <div className="room-admin-header">
                ...
                <div className="action-btns">
                  ...
                  >
                    ...
                    {loading === room.id + '_del' ? 'Eliminando...' : 'Eliminar'}
                  </button>
                </div>
              </div>
                    ...
                    <div className="detail-sections">
                      <div className="detail-section">
                        <div className="detail-section-header">
                          ...
                          <Link href={`/salas/${room.slug}`} className="detail-link" target="_blank">Ver sala →</Link>
                        </div>
                              ...
                              <div key={topic.id} className={`detail-item ${topic.is_locked ? 'locked' : ''}`}>
                                <div className="detail-item-info">
                                  <div className="detail-item-title-row">
                                    ...
                                    {topic.has_blocked_posts && <ExclamationTriangleIcon style={{ width: 11, height: 11, color: '#ff6b6b', flexShrink: 0 }} />}
                                    <Link href={`/salas/${room.slug}/${topic.id}`} className="detail-item-name" target="_blank">{topic.title}</Link>
                                  </div>
                                  <div className="detail-item-meta">
                                    <span>{topic.post_count ?? 0} posts</span>
                                    {topic.author && <span>por {topic.author.display_name || topic.author.username}</span>}
                                    {topic.last_post && (
                                      <span>último: {topic.last_post.author?.display_name || topic.last_post.author?.username ?? '—'} · {new Date(topic.last_post.created_at).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })}</span>
                                    )}
                                  </div>
                                </div>
                                <div className="detail-item-actions">
                                  <button className={`detail-btn ${topic.is_pinned ? 'active' : ''}`} onClick={() => handlePinTopic(topic.id, topic.is_pinned, room.id, room.slug)} disabled={loading === 'pin_' + topic.id} title={topic.is_pinned ? 'Desfijar' : 'Fijar'}>
                                    <MapPinIcon style={{ width: 11, height: 11 }} />
                                  </button>
                                  <button className={`detail-btn ${topic.is_locked ? 'warn' : ''}`} onClick={() => handleLockTopic(topic.id, topic.is_locked, room.id, room.slug)} disabled={loading === 'topic_' + topic.id} title={topic.is_locked ? 'Desbloquear' : 'Bloquear'}>
                                    {topic.is_locked ? <LockOpenIcon style={{ width: 11, height: 11 }} /> : <LockClosedIcon style={{ width: 11, height: 11 }} />}
                                  </button>
                                  <button className="detail-btn danger" onClick={() => handleDeleteTopic(topic.id, room.id, room.slug)} disabled={loading === 'del_topic_' + topic.id} title="Eliminar tema">
                      ...
                      <div className="detail-section">
                        <div className="detail-section-header">
                          ...
                          <Link href={`/salas/${room.slug}/wiki`} className="detail-link" target="_blank">Ver wiki →</Link>
                        </div>
                              ...
                              <div key={wiki.id} className="detail-item">
                                <div className="detail-item-info">
                                  <div className="detail-item-title-row">
                                    ...
                                    <Link href={`/salas/${room.slug}/wiki/${wiki.slug}`} className="detail-item-name" target="_blank">{wiki.title}</Link>
                                  </div>
                                  <div className="detail-item-meta">
                                ...
                                <div className="detail-item-actions">
                                  <Link href={`/salas/${room.slug}/wiki/${wiki.slug}/editar`} className="detail-btn" target="_blank" title="Editar">✎</Link>
                                  <button className="detail-btn danger" onClick={() => handleDeleteWiki(wiki.id, room.id, room.slug)} disabled={loading === 'del_wiki_' + wiki.id} title="Eliminar página">
                                    <TrashIcon style={{ width: 11, height: 11 }} />
                                  </button>
                                </div>
                              </div>
