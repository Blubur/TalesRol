...
export default function RegisterPage() {
  ...
  // Comprobar si el modo invitación está activo
  useEffect(() => {
    fetch('/api/config/invite-only')
      .then(r => r.json())
      .then(d => setInviteOnly(d.inviteOnly === true))
      .catch(() => {})
  }, [])
  ...
  return (
    <div className="auth-page">
        ...
        <form action={handleSubmit} className="auth-form">
          ...
          <div className="form-group">
            ...
            <input
              ...
              placeholder="El Cronista" className="input-base"
          ...
          {/* Campo de código de invitación — solo si invite_only está activo */}
          {inviteOnly && (
            <div className="form-group">
              ...
              />
              <span className="field-hint">
                El registro está restringido. Necesitas un código para unirte.
              </span>
            </div>
          )}
      ...
      <style>{`
        .auth-page { min-height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:2rem 1rem; position:relative; overflow:hidden; }
        .auth-bg-overlay { position:fixed; inset:0; background: radial-gradient(ellipse 60% 40% at 50% 0%, rgba(193,6,6,0.12) 0%, transparent 60%), radial-gradient(ellipse 40% 60% at 20% 80%, rgba(193,6,6,0.06) 0%, transparent 50%); pointer-events:none; z-index:0; }
        .auth-logo { text-align:center; margin-bottom:2rem; position:relative; z-index:1; display:flex; flex-direction:column; align-items:center; }
        .auth-logo-symbol { font-size:2rem; color:var(--color-crimson); display:block; margin-bottom:0.5rem; animation:flicker 3s infinite; }
        .auth-logo-text { font-family:var(--font-cinzel); font-size:2.5rem; font-weight:900; letter-spacing:0.15em; text-transform:uppercase; background:linear-gradient(135deg,#ff4444 0%,#C10606 50%,#8a0404 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; margin:0; line-height:1; }
        .auth-logo-sub { color:var(--text-muted); font-size:0.8rem; letter-spacing:0.2em; text-transform:uppercase; margin-top:0.4rem; font-family:var(--font-cinzel); }
        .auth-card { background:var(--bg-card); border:1px solid var(--border-subtle); border-radius:6px; padding:2.5rem; width:100%; max-width:440px; position:relative; z-index:1; box-shadow:0 20px 60px rgba(0,0,0,0.5),0 0 40px rgba(193,6,6,0.06); }
        .auth-card-header { text-align:center; margin-bottom:2rem; }
        .auth-card-header h2 { font-family:var(--font-cinzel); font-size:1.4rem; font-weight:600; letter-spacing:0.1em; color:var(--text-primary); margin:0 0 0.4rem; }
        ...
        .auth-terms { text-align:center; color:var(--text-muted); font-size:0.82rem; margin:-0.5rem 0 0; }
        .auth-terms a { color:var(--text-secondary); }
        .auth-terms a:hover { color:var(--color-crimson); }
        .auth-switch { text-align:center; color:var(--text-muted); font-size:0.9rem; margin:0; }
